    ...
    async runAttack({ prompt, filters, vars, provider, context, options, test, }) {
        ...
        `;
                ...
                if (test && assertToUse) {
                    ...
                    if (grader) {
                        const { grade } = await grader.getResult(this.userGoal, lastResponse.output, test, provider, assertToUse && 'value' in assertToUse ? assertToUse.value : undefined);
    ...
    async getAttackPrompt(roundNum, evalFlag, lastResponse, lastFeedback, objectiveScore) {
        ...
        for (const key of expectedKeys) {
            if (!(key in parsedOutput)) {
                logger_1.default.warn(`[Crescendo] Missing key in response: ${key}`);
            }
        }

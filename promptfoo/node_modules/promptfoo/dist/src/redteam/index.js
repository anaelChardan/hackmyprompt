 ...
 */
async function applyStrategies(testCases, strategies, injectVar) {
    const newTestCases = [];
    ...
    for (const strategy of strategies) {
        ...
        const strategyTestCases = await strategyAction(applicableTestCases, injectVar, strategy.config || {});
 ...
 */
async function synthesize({ abortSignal, delay, entities: entitiesOverride, injectVar, language, maxConcurrency = 1, plugins, prompts, provider, purpose: purposeOverride, strategies, targetLabels, showProgressBar: showProgressBarOverride, }) {
    ...
    checkAbort();
    const { testCases: strategyTestCases, strategyResults: otherStrategyResults } = await applyStrategies(pluginTestCases, strategies.filter((s) => !['basic', 'multilingual', 'retry'].includes(s.id)), injectVar);

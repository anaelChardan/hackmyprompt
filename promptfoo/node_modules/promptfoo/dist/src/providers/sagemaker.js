...
exports.SageMakerEmbeddingProvider = exports.SageMakerCompletionProvider = exports.SageMakerGenericProvider = void 0;
 ...
 */
class SageMakerGenericProvider {
    ...
    async getCredentials() {
        if (this.config.accessKeyId && this.config.secretAccessKey) {
            logger_1.default.debug('Using explicit credentials from config');
            return {
        ...
        if (this.config.profile) {
            logger_1.default.debug(`Using AWS profile: ${this.config.profile}`);
            try {
        ...
        logger_1.default.debug('Using default AWS credentials from environment');
        return undefined;
    }
    /**
    ...
    async getSageMakerRuntimeInstance() {
        if (!this.sagemakerRuntime) {
            try {
                const { SageMakerRuntimeClient } = await Promise.resolve().then(() => __importStar(require('@aws-sdk/client-sagemaker-runtime')));
                ...
                logger_1.default.debug(`SageMaker client initialized for region ${this.getRegion()}`);
            }
            catch {
    ...
    getRegion() {
        return (this.config?.region ||
            this.env?.AWS_REGION ||
            (0, envars_1.getEnvString)('AWS_REGION') ||
            (0, envars_1.getEnvString)('AWS_DEFAULT_REGION') ||
            'us-east-1');
    }
    /**
    ...
    async applyTransformation(prompt, context) {
        ...
        try {
            ...
            logger_1.default.debug(`Applying transform to prompt for SageMaker endpoint ${this.getEndpointName()}`);
                ...
                try {
                    ...
                    if (transformFn.includes('=>')) {
                        ...
                        if (result === undefined || result === null) {
                            ...
                            logger_1.default.debug('Transform function returned null or undefined, using original prompt');
                            return prompt;
                        }
                    ...
                    logger_1.default.error(`Error executing inline transform: ${transformError}`);
                }
            }
            else {
                // Use the transform utility for file transforms
                try {
                    const { TransformInputType } = await Promise.resolve().then(() => __importStar(require('../util/transform')));
                    const transformed = await (0, transform_1.transform)(transformFn, prompt, transformContext, false, TransformInputType.OUTPUT);
                    // Handle all possible return types, including falsy values (empty string, 0, etc.)
                    ...
                    logger_1.default.error(`Error using transform utility: ${transformError}`);
                }
            }
            // Fall back to the original prompt if the transform result is not usable
            logger_1.default.warn(`Transform did not produce a valid result, using original prompt`);
            return prompt;
        }
        catch (_) {
            logger_1.default.error(`Error applying transform to prompt: ${_}`);
        ...
        try {
            ...
            if (pathExpression.startsWith('file://')) {
                try {
                    ...
                    const { TransformInputType } = await Promise.resolve().then(() => __importStar(require('../util/transform')));
                    const transformedResult = await (0, transform_1.transform)(pathExpression, responseJson, { prompt: {} }, // Minimal context since we're just transforming the response
                    ...
                    logger_1.default.warn(`Failed to transform response using file: ${error}`);
                    return responseJson;
                }
            }
            ...
            try {
                ...
                if (result === undefined) {
                    logger_1.default.warn(`Path expression "${pathExpression}" did not match any data in the response`);
                    logger_1.default.debug(`Response JSON structure: ${JSON.stringify(responseJson).substring(0, 200)}...`);
                    return responseJson;
                }
                return result;
            }
            catch (error) {
                logger_1.default.warn(`Failed to evaluate expression "${pathExpression}": ${error}`);
                return responseJson;
            }
        }
        catch (error) {
            logger_1.default.warn(`Failed to extract data using path expression "${pathExpression}": ${error}`);
            logger_1.default.debug(`Response JSON structure: ${JSON.stringify(responseJson).substring(0, 200)}...`);
            return responseJson;
        }
    }
}
exports.SageMakerGenericProvider = SageMakerGenericProvider;
/**
 * Provider for text generation with SageMaker endpoints
 */
class SageMakerCompletionProvider extends SageMakerGenericProvider {
    /**
     * Format the request payload based on model type
     */
    formatPayload(prompt) {
        const modelType = this.config.modelType || 'custom';
        const maxTokens = this.config.maxTokens || (0, envars_1.getEnvInt)('AWS_SAGEMAKER_MAX_TOKENS') || 1024;
        const temperature = this.config.temperature || (0, envars_1.getEnvFloat)('AWS_SAGEMAKER_TEMPERATURE') || 0.7;
        const topP = this.config.topP || (0, envars_1.getEnvFloat)('AWS_SAGEMAKER_TOP_P') || 1.0;
        const stopSequences = this.config.stopSequences || [];
        let payload;
        logger_1.default.debug(`Formatting payload for model type: ${modelType}`);
        switch (modelType) {
            case 'openai':
                try {
    ...
    async parseResponse(responseBody) {
        const modelType = this.config.modelType || 'custom';
        let responseJson;
        logger_1.default.debug(`Parsing response for model type: ${modelType}`);
        try {
            responseJson = JSON.parse(responseBody);
        }
        catch {
            logger_1.default.debug('Response is not JSON, returning as-is');
        ...
        if (this.config.responseFormat?.path) {
            try {
                ...
                logger_1.default.warn(`Failed to extract from path: ${this.config.responseFormat.path}, Error: ${error}`);
                logger_1.default.debug(`Response JSON structure: ${JSON.stringify(responseJson).substring(0, 200)}...`);
                return responseJson;
            }
        }
        ...
        if (responseJson.generated_text) {
            logger_1.default.debug('Detected JumpStart model response format with generated_text field');
    ...
    getCacheKey(prompt) {
        ...
        try {
            ...
            logger_1.default.debug(`Unable to create hash for cache key: ${_}`);
    ...
    async callApi(prompt, context, options) {
        ...
        const { isCacheEnabled, getCache } = await Promise.resolve().then(() => __importStar(require('../cache')));
        ...
        if (isTransformed) {
            logger_1.default.debug(`Prompt transformed for SageMaker endpoint ${this.getEndpointName()}`);
            logger_1.default.debug(`Original: ${prompt.substring(0, 100)}${prompt.length > 100 ? '...' : ''}`);
            logger_1.default.debug(`Transformed: ${transformedPrompt.substring(0, 100)}${transformedPrompt.length > 100 ? '...' : ''}`);
        }
        ...
        if (isCacheEnabled() && !bustCache) {
            ...
            const cache = (await getCache)
                ? await getCache()
                : await Promise.resolve().then(() => __importStar(require('../cache'))).then((m) => m.getCache());
            // Try to get from cache
            const cachedResult = await cache.get(cacheKey);
            if (cachedResult) {
                logger_1.default.debug(`Using cached SageMaker response for ${this.getEndpointName()}`);
                try {
                    ...
                    logger_1.default.warn(`Failed to parse cached SageMaker response: ${_}`);
        ...
        if (delayMs && delayMs > 0) {
            logger_1.default.debug(`Applying delay of ${delayMs}ms before calling SageMaker endpoint ${this.getEndpointName()}`);
            await sleep(delayMs);
        }
        ...
        logger_1.default.debug(`Calling SageMaker endpoint ${this.getEndpointName()}`);
        logger_1.default.debug(`With payload: ${payload.length > 1000 ? payload.substring(0, 1000) + '...' : payload}`);
        try {
            const { InvokeEndpointCommand } = await Promise.resolve().then(() => __importStar(require('@aws-sdk/client-sagemaker-runtime')));
            ...
            if (!response.Body) {
                logger_1.default.error('No response body returned from SageMaker endpoint');
                return {
                    error: 'No response body returned from SageMaker endpoint',
                };
            }
            const responseBody = new TextDecoder().decode(response.Body);
            logger_1.default.debug(`SageMaker response (truncated): ${responseBody.length > 1000 ? responseBody.substring(0, 1000) + '...' : responseBody}`);
            ...
            if (isCacheEnabled() && !bustCache && result.output && !result.error) {
                ...
                try {
                    ...
                    logger_1.default.debug(`Stored SageMaker response in cache with key: ${cacheKey.substring(0, 100)}...`);
                }
                catch (_) {
                    logger_1.default.warn(`Failed to store SageMaker response in cache: ${_}`);
                }
            }
            return result;
        }
        catch (error) {
            logger_1.default.error(`SageMaker API error: ${error}`);
            return {
...
SageMakerCompletionProvider.SAGEMAKER_MODEL_TYPES = [
    'openai',
    'anthropic',
    'llama',
    'huggingface',
 ...
 */
class SageMakerEmbeddingProvider extends SageMakerGenericProvider {
    async callApi() {
    ...
    getCacheKey(text) {
        ...
        try {
            ...
            // Fall back to the unoptimized version if crypto is not available
            logger_1.default.debug(`Unable to create hash for embedding cache key: ${_}`);
    ...
    async callEmbeddingApi(text, context) {
        ...
        if (isTransformed) {
            logger_1.default.debug(`Text transformed for SageMaker embedding endpoint ${this.getEndpointName()}`);
            logger_1.default.debug(`Original: ${text.substring(0, 100)}${text.length > 100 ? '...' : ''}`);
            logger_1.default.debug(`Transformed: ${transformedText.substring(0, 100)}${transformedText.length > 100 ? '...' : ''}`);
        }
        ...
        if (isCacheEnabled() && !bustCache) {
            ...
            const cachedResult = await cache.get(cacheKey);
            if (cachedResult) {
                logger_1.default.debug(`Using cached SageMaker embedding response for ${this.getEndpointName()}`);
                try {
                    // Parse the cached result
                    const parsedResult = JSON.parse(cachedResult);
                    // Add cache flag to token usage
                    ...
                    logger_1.default.warn(`Failed to parse cached SageMaker embedding response: ${_}`);
                    // Continue with API call if parsing fails
                }
            }
        }
        // Apply delay if specified and not using cached response
        if (delayMs && delayMs > 0) {
            logger_1.default.debug(`Applying delay of ${delayMs}ms before calling SageMaker embedding endpoint ${this.getEndpointName()}`);
            await sleep(delayMs);
        }
        // Not in cache or cache disabled, make the actual API call
        const runtime = await this.getSageMakerRuntimeInstance();
        let payload;
        const modelType = this.config.modelType || 'custom';
        logger_1.default.debug(`Formatting embedding payload for model type: ${modelType}`);
        switch (modelType) {
            case 'openai':
        ...
        logger_1.default.debug(`Calling SageMaker embedding endpoint ${this.getEndpointName()}`);
        logger_1.default.debug(`With payload: ${payload}`);
        try {
            const { InvokeEndpointCommand } = await Promise.resolve().then(() => __importStar(require('@aws-sdk/client-sagemaker-runtime')));
            const command = new InvokeEndpointCommand({
                EndpointName: this.getEndpointName(),
            ...
            const _latency = endTime - startTime;
            if (!response.Body) {
                logger_1.default.error('No response body returned from SageMaker embedding endpoint');
                return {
                    error: 'No response body returned from SageMaker embedding endpoint',
                };
            }
            const responseBody = new TextDecoder().decode(response.Body);
            logger_1.default.debug(`SageMaker embedding response: ${responseBody.substring(0, 200)}...`);
            let responseJson;
            try {
                responseJson = JSON.parse(responseBody);
            }
            ...
            if (this.config.responseFormat?.path) {
                try {
                    ...
                    if (Array.isArray(extracted) && extracted.every((val) => typeof val === 'number')) {
                        ...
                        logger_1.default.warn('Extracted data is not a valid embedding array, trying other extraction methods');
                    }
                }
                catch (error) {
                    logger_1.default.warn(`Failed to extract embedding from path expression: ${this.config.responseFormat.path}, Error: ${error}`);
                    logger_1.default.debug(`Response JSON structure: ${JSON.stringify(responseJson).substring(0, 200)}...`);
            ...
            logger_1.default.error(`SageMaker embedding API error: ${error}`);
            return {
     ...
     */
    async cacheEmbeddingResult(result, text, // This is the transformed text
    context, isTransformed = false, originalText) {
        const { isCacheEnabled, getCache } = await Promise.resolve().then(() => __importStar(require('../cache')));
        ...
        if (isCacheEnabled() && !bustCache && result.embedding && !result.error) {
            ...
            try {
                await cache.set(cacheKey, resultToCache);
                logger_1.default.debug(`Stored SageMaker embedding response in cache with key: ${cacheKey.substring(0, 100)}...`);
            }
            catch (_) {
                logger_1.default.warn(`Failed to store SageMaker embedding response in cache: ${_}`);
            }
        }
    }
}

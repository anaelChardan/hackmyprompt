...
class OpenAiChatCompletionProvider extends _1.OpenAiGenericProvider {
    ...
    getOpenAiBody(prompt, context, callApiOptions) {
        ...
        const body = {
            ...
            ...(config.functions
                ? {
                    functions: (0, util_2.maybeLoadFromExternalFile)((0, util_1.renderVarsInObject)(config.functions, context?.vars)),
                }
                : {}),
            ...(config.function_call ? { function_call: config.function_call } : {}),
            ...(config.tools
                ? { tools: (0, util_2.maybeLoadFromExternalFile)((0, util_1.renderVarsInObject)(config.tools, context?.vars)) }
                : {}),
            ...(config.tool_choice ? { tool_choice: config.tool_choice } : {}),
            ...(config.tool_resources ? { tool_resources: config.tool_resources } : {}),
            ...(config.response_format
                ? {
                    response_format: (0, util_2.maybeLoadFromExternalFile)((0, util_1.renderVarsInObject)(config.response_format, context?.vars)),
                }
                : {}),
    ...
    async callApi(prompt, context, callApiOptions) {
        ...
        if (data.error) {
            await data.deleteFromCache?.();
            return {
                error: (0, util_4.formatOpenAiError)(data),
            };
        }
        try {
            ...
            if (message.refusal) {
                return {
                    ...
                    tokenUsage: (0, util_4.getTokenUsage)(data, cached),
                    isRefusal: true,
                };
            }
            let output = '';
            ...
            if (functionCalls && config.functionToolCallbacks) {
                ...
                if (results.length > 0) {
                    return {
                        ...
                        cached,
                        logProbs,
                        cost: (0, util_3.calculateOpenAICost)(this.modelName, config, data.usage?.prompt_tokens, data.usage?.completion_tokens, data.usage?.audio_prompt_tokens, data.usage?.audio_completion_tokens),
                    };
                }
            }
...
OpenAiChatCompletionProvider.OPENAI_CHAT_MODELS = util_4.OPENAI_CHAT_MODELS;
OpenAiChatCompletionProvider.OPENAI_CHAT_MODEL_NAMES = util_4.OPENAI_CHAT_MODELS.map((model) => model.id);

    ...
    if (typeof parser === 'function') {
        return (data, text, context) => {
            try {
                ...
                logger_1.default.error(`Error in response transform function: ${String(err)}`);
                throw err;
            }
        };
    }
    if (typeof parser === 'string' && parser.startsWith('file://')) {
        ...
        return (data, text, context) => {
            try {
                ...
                logger_1.default.error(`Error in response transform: ${String(err)}`);
...
class HttpProvider {
    constructor(url, options) {
        ...
        if (this.config.request) {
            this.config.request = (0, util_1.maybeLoadFromExternalFile)(this.config.request);
        }
        else {
    ...
    validateContentTypeAndBody(headers, body) {
        if (body != null) {
            ...
            if (typeof body === 'string' && contentTypeIsJson(headers)) {
                logger_1.default.warn('Content-Type is application/json, but body is a string. This is likely to cause unexpected results. It should be an object or array.');
            }
        }
    }
    ...
    async callApiWithRawRequest(vars, context) {
        ...
        const renderedRequest = (0, templates_1.getNunjucksEngine)().renderString(this.config.request, vars);

...
exports.AzureAssistantProvider = void 0;
...
class AzureAssistantProvider extends generic_1.AzureGenericProvider {
    ...
    async loadExternalFunction(fileRef) {
        ...
        if (filePath.includes(':')) {
            ...
            if (splits[0] && (0, file_1.isJavascriptFile)(splits[0])) {
    ...
    async callApi(prompt, context, callApiOptions) {
        ...
        const cacheKey = `azure_assistant:${this.deploymentName}:${JSON.stringify({
            ...
            tools: JSON.stringify((0, util_1.maybeLoadFromExternalFile)((0, util_1.renderVarsInObject)(this.assistantConfig.tools, context?.vars))),
            top_p: this.assistantConfig.top_p,
        })}`;
        // Check the cache if enabled
        if ((0, cache_2.isCacheEnabled)()) {
            try {
                const cache = await (0, cache_2.getCache)();
                const cachedResult = await cache.get(cacheKey);
                if (cachedResult) {
                    logger_1.default.debug(`Cache hit for assistant prompt: ${prompt.substring(0, 50)}...`);
        ...
        try {
            ...
            logger_1.default.debug(`Created thread ${threadResponse.id} for prompt: ${prompt.substring(0, 30)}...`);
            // Create a message
            ...
            if (this.assistantConfig.tools) {
                runOptions.tools = (0, util_1.maybeLoadFromExternalFile)((0, util_1.renderVarsInObject)(this.assistantConfig.tools, context?.vars));
            }
            ...
            if (this.assistantConfig.functionToolCallbacks &&
                ...
                result = await this.pollRunWithToolCallHandling(apiBaseUrl, apiVersion, threadResponse.id, runResponse.id);
            }
            else {
                // Poll for completion
                ...
                if (completedRun.status === 'completed') {
                    result = await this.processCompletedRun(apiBaseUrl, apiVersion, threadResponse.id, completedRun);
                }
                else {
            ...
            if ((0, cache_2.isCacheEnabled)() && !result.error) {
                try {
                    const cache = await (0, cache_2.getCache)();
                    await cache.set(cacheKey, result);
                    logger_1.default.debug(`Cached assistant response for prompt: ${prompt.substring(0, 50)}...`);
                }
                catch (err) {
            ...
            logger_1.default.error(`Error in Azure Assistant API call: ${err}`);
    ...
    async makeRequest(url, options) {
        const timeoutMs = this.assistantConfig.timeoutMs || shared_1.REQUEST_TIMEOUT_MS;
        ...
        try {
            const result = await (0, cache_1.fetchWithCache)(url, options, timeoutMs, 'json', shouldBustCache, retries);
            ...
            if (result.status < 200 || result.status >= 300) {
                ...
                throw new Error(`API error: ${result.status} ${result.statusText}${result.data && typeof result.data === 'object' && 'error' in result.data
            ...
            logger_1.default.error(`Request failed: ${error.message}`);
            throw error;
        }
    }
    /**
     ...
     */
    async pollRun(apiBaseUrl, apiVersion, threadId, runId, pollIntervalMs = 1000) {
        ...
        while (['queued', 'in_progress'].includes(runStatus.status)) {
            ...
            if (Date.now() - startTime > maxPollTime) {
                throw new Error(`Run polling timed out after ${maxPollTime}ms. Last status: ${runStatus.status}`);
            }
                    ...
                    if (run.required_action?.type === 'submit_tool_outputs' &&
                        ...
                        const toolOutputs = await Promise.all(functionCallsWithCallbacks.map(async (toolCall) => {
                            ...
                            try {
                                ...
                                const outputResult = await this.executeFunctionCallback(functionName, functionArgs);
                        ...
                        logger_1.default.debug(`Submitting tool outputs: ${JSON.stringify(validToolOutputs)}`);
                        // Submit tool outputs
                        try {
                            await this.makeRequest(`${apiBaseUrl}/openai/threads/${threadId}/runs/${runId}/submit_tool_outputs?api-version=${apiVersion}`, {
                                method: 'POST',
                        ...
                        logger_1.default.error(`Unknown required action type: ${run.required_action?.type}`);
                        break;
                    }
                }
    ...
    async processCompletedRun(apiBaseUrl, apiVersion, threadId, runIdOrResponse) {
        try {
            ...
            if (typeof runIdOrResponse === 'string') {
                await this.makeRequest(`${apiBaseUrl}/openai/threads/${threadId}/runs/${runId}?api-version=${apiVersion}`, {
                    method: 'GET',
                    headers: await this.getHeaders(),
                });
            }
...
exports.AzureAssistantProvider = AzureAssistantProvider;

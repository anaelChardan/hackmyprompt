...
async function doEval(cmdObj, defaultConfig, defaultConfigPath, evaluateOptions) {
    ...
    const runEvaluation = async (initialization) => {
        ...
        if (testSuite.scenarios) {
            testSuite.scenarios = (await (0, util_1.maybeLoadFromExternalFile)(testSuite.scenarios));
        }
        for (const scenario of testSuite.scenarios || []) {
            if (scenario.tests) {
                scenario.tests = await (0, util_1.maybeLoadFromExternalFile)(scenario.tests);
            }
        }
      ...
      Please review your promptfooconfig.yaml configuration.`));
        ...
        const shareableUrl = cmdObj.share && config.sharing ? await (0, share_1.createShareableUrl)(evalRecord) : null;
        let successes = 0;
        let failures = 0;
        let errors = 0;
        const tokenUsage = {
